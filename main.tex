...
Our methodology generates an average validation split challenge score of 0.486, with a PhysioNet official phase hold out test set score of \officialscore under the team name, CVC.
...
Each record contains a set of one or more SNOMED CT codes, although only a subset of 27 codes are evaluated in the challenge.
...
% \end{figure}
Refer to \cite{physionet_challenge_2020} for the description of the challenge scoring function weights and ECG dataset.
...
The heartbeat template category containing 536 features is the most numerous feature type. There are 291 full waveform features and 172 heart rate variability features.
The age meta feature that is parsed from the ECG record header is also incorporated.
...
Our model's top three best classified labels are normal sinus rhythm (SNR, $\text{F}_1$ mean: 0.924), left bundle branch block (LBBB, $\text{F}_1$ mean: 0.840), and sinus tachycardia (STach, $\text{F}_1$ mean: 0.777).
...
We acknowledge that our results report optimistic values for the classification metrics, as our internal split of the dataset does not include a hold-out test set.
